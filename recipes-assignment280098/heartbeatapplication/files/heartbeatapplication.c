...
int main(int argc, char **argv)
{
	...
	printf("Heartbeat monitoring started...\n");
	...
	while(1) {
		...
		for(i=0; i<N; i++) {
			...
			pthread_join(myThread, NULL); //the thread is expected to terminate before this function is called.
						      //So I call it to clean up the memory used by the thread before launching a new one.
						      //I also avoid race conditions, as two threads won't be running in parallel.
		}
